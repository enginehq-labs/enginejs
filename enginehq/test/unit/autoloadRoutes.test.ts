...
test('autoloadRoutes: recursive file-based routing', async (t) => {
  ...
  assert.ok(paths.some((p: string) => p.includes('api\\/users') && p.includes('([^/]+?)')), 'missing /api/users/:id');
  assert.ok(paths.some((p: string) => p.includes('api\\/posts') && p.includes('([^/]+?)') && p.includes('comments')), 'missing /api/posts/:slug/comments');
