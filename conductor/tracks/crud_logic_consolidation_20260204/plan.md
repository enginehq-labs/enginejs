...
- [ ] Task: Move payload processing utilities to `@enginehq/core`.
    - [ ] Create `core/src/crud/utils.ts` and implement `pruneUnknownPayload`, `stripVirtualFields`, `parseArrayish`, and `normalizePayloadMultiFields`.
    - [ ] Write unit tests in `core/test/unit/crud/utils.test.ts`.
- [ ] Task: Integrate utilities into `CrudService`.
    - [ ] Update `CrudService` to use these utilities for all mutation operations (`create`, `update`).
