...
test('docker postgres: workflow specs can be loaded from DB registry', async (t) => {
  ...
  assert.equal(events.length, 2, 'Should emit post and comment events');
  ...
  assert.equal(commentEvent?.status, 'pending', 'The runner has not picked up the comment event yet');
